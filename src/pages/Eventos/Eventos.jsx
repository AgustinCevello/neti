...
function validateForm({ nombre, apellido, email }) {
    ...
    else if (!soloLetras.test(nombre)) errors.nombre = 'Solo se permiten letras';
    else if (nombre.length > 30) errors.nombre = 'Máximo 30 caracteres';

    if (!apellido.trim()) errors.apellido = 'El apellido es requerido';
    else if (!soloLetras.test(apellido)) errors.apellido = 'Solo se permiten letras';
...
function InscripcionForm() {
    ...
    const handleChange = (e) => {
        ...
        // Bloquear caracteres no permitidos en nombre/apellido mientras se escribe
        if ((name === 'nombre' || name === 'apellido') && !soloLetrasRegex.test(value)) return;
    ...
    const handleSubmit = (e) => {
        ...
        if (Object.keys(errs).length > 0) {
            setErrors(errs);
            // Quitar foco para que NO se dispare la animación de enviado
            btnRef.current?.blur();
            return;
        }
    ...
    if (submitted) {
        return (
            <div
                ...
                style={{
                    animation: 'thanksFadeIn 0.7s cubic-bezier(0.34,1.56,0.64,1) forwards',
                    opacity: 0,
                }}
            >
                <div
                    className="w-16 h-16 rounded-full bg-[#EC4E8D] flex items-center justify-center mx-auto mb-4"
                ...
                <h3
                    className="font-sans font-bold text-[#251B37] text-xl mb-2"
                    style={{ animation: 'thanksFadeIn 0.5s ease 0.3s forwards', opacity: 0 }}
                >
                    ¡Gracias por inscribirte!
                </h3>
                <p
                    className="font-sans text-[#85789A] text-sm"
    ...
    const fields = [
        { id: 'nombre', label: 'Nombre', type: 'text', placeholder: 'Tu nombre', maxLen: 30, showCounter: true },
        { id: 'apellido', label: 'Apellido', type: 'text', placeholder: 'Tu apellido', maxLen: 30, showCounter: true },
        { id: 'email', label: 'E-mail', type: 'email', placeholder: 'tu@email.com', maxLen: 100, showCounter: false },
    ];

    return (
        <form onSubmit={handleSubmit} noValidate>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                {fields.map(({ id, label, type, placeholder, maxLen, showCounter }) => (
                    <div key={id}>
                        ...
                        <div className="flex justify-between items-start mt-1 min-h-[18px]">
                            {errors[id]
                                ? <p className="font-sans text-xs text-red-400">{errors[id]}</p>
                                : <span />
                            }
                            {showCounter && form[id].length > 0 && (
                                <span className="font-sans text-[11px] text-[#85789A]/50 ml-auto">
                                    {form[id].length}/{maxLen}
                                </span>
                            )}
                        </div>
                    </div>
            ...
            <div className="text-center">
                <button
                    ref={btnRef}
                    type="submit"
                    className={`uiverse-btn${sent ? ' uiverse-btn--sent' : ''}`}
        ...
        <div className="bg-white overflow-x-hidden">
            ...
            <section className="py-16 px-4 md:px-8">
                <div className="max-w-5xl mx-auto">
                    <FadeIn>
                        <h2 className="font-display text-4xl md:text-5xl font-black uppercase mb-10"
                            style={{ color: '#35112F' }}>
                            Actividades
                        </h2>
                    </FadeIn>

                    {/* Carrusel simple de actividades */}
            ...
            <section className="py-16 px-4 md:px-8">
                <div className="max-w-5xl mx-auto">
                    <FadeIn>
                        <h2 className="font-display text-4xl md:text-5xl font-black uppercase mb-14"
                            style={{ color: '#35112F' }}>
                            Oradores
                        </h2>
                    </FadeIn>
            ...
            <section className="py-16 px-4 md:px-8">
                <div className="max-w-5xl mx-auto">
                    <FadeIn>
                        <h2 className="font-display text-4xl md:text-5xl font-black uppercase mb-12"
                            style={{ color: '#35112F' }}>
                            Organizadores
                        </h2>
                    </FadeIn>
